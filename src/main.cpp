...
// Copyright (c) 2014 The WeCoin developers
...
uint256 hashGenesisBlock("0x000000c47404e61fae2b61a1216108720186e3bda6c88130912617a18d7d6e88");
static const unsigned int timeGenesisBlock = 1390822264;
static const unsigned int nNonceGenesisBlock = 31712844;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "Polacca werwolf ericoid ectromelia undersprout stiegel raymondville";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x0609c30b3a36b5638b0487ab96a2b120492d940a0232b5fd7d5c70e167d0e342"));
...
unsigned char pchMessageStart[4] = { 0xbe, 0xf9, 0xd2, 0xbb };
...
//        nBlockMaxSize = std::min(nBlockMaxSize, (unsigned int)(MAX_BLOCK_SIZE_GEN));
    ...
    int64 nFees = 0;
    {
        ...
        pblock->vtx[0].vout[0].nValue = GetBlockValue(pindexPrev->nHeight+1, nFees, pblock->nBits, pindexPrev->GetBlockHash());
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WeCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WeCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("WeCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wecoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running WeCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
